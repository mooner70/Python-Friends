...
def registration(request):
    ...
    if len(errors):
        ...
        b = User.objects.create(name=request.POST["name"], alais=request.POST["alais"], email=request.POST["email"], password=hash1, dob=request.POST["dob"])
        request.session['alais'] = request.POST["alais"]
...
def login(request):
    ...
    if len(errors):
        ...
        request.session['alais'] = user.alais
